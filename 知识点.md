...
2. 四次挥手
   ...
   ![](image\interview\sevenceng.png)	![](image\interview\thirdhandshake.png)
...
12. 简单谈下channel的应用场景及注意事项
    ...
    | 场景                   | 读取             | 写入           | 关闭  |
    | ---------------------- | ---------------- | -------------- | ----- |
    | 未初始化chan           | 死锁（deadlock） | 死锁(deadlock) | panic |
...
它通过握手开始时SYN包中的TFO cookie 来验证一个之前连接过的客户端。如果验证成功，他可以在三次握手最终的ACK包收到之前就开始发送数据。这个加密的cookie被存储在客户端，在一开始的连接时被设定好。
...
- 此条目不提交前不会变更leader的值 ，为了提交条目，leader首先复制它到其他的Follower 节点， 然后leader等直到多数节点被写入条目，该条目现在提交给leader节点 leader状态发生变更，然后leader通知followers 这个条目已经提交， 然后节点发生变更 此时系统是一致性的。这个过程叫做log replication
...
- 203 当代理服务器修改了origin server的原始响应包体时 代理服务器修改200为203
...
互斥锁加锁失败而阻塞的现象，是由操作系统内核实现 当加锁失败后， 内核将线程置位睡眠状态，等到锁被释放后，内核会在合适的时机唤醒线程，当这个线程成功获取锁后， 于是就可以继续执行 。
...
- 两个以上的go程中，使用多个channel通信， A go程获取channel1的同时，尝试使用channel2 ，同一时刻，B go程获取channel2的同时 尝试使用channel1
...
- **ps**
  ...
  - linux进程5种状态：
    ...
    - 不可中断（收到信号不唤醒或不可运行，进程必须等待直到有中断发送）
    - 僵死（进程已结束，但进程描述符存在，直到父进程调用wait4系统调用后释放）
  ...
  - ps -aux 命令 Head标头的意义：
    ...
    - RSS 进程使用的实际内存大小
    - STAT 进程状态
      - R运行 S睡眠 I空闲 Z僵死 D不可中断 T终止 <高优先级 N低优先级

- **free** buffer/cache 作用：buffer用于存放要输出到disk的数据 cache存放从disk读出的数据 二者是为了提高IO性能 由OS管理

- **df和du不一致的原因**：du使用fstat系统调用，df使用statfs系统调用， df和du不一致原因在于文件删除问题。当文件删除后 文件系统目录中不可见， du不会在统计，而如果此时还有运行的进程持有这个已经删除文件的句柄，那么这个文件就不会真正在磁盘中删除 df仍会统计。

  解决办法： ```lsof | grep delete``` --查看已删除 仍被进程占用没有释放的文件相关信息

## 15.ETCD备份命令

备份命令：```etcdctl --endpoints localhost:2379 snapshot save backup.db```

恢复命令：```etcdctl snapshot restore backup.db --name m3 --data-dir=/home/etcd_data```
...
2. docker overlay overlay2 区别

3. etcd写入瓶颈（event事件）

4. notready状态的node 为什么可以调度daemonset （calico agent）污点和容忍度

5. calico 通信过程：

   参考文章：https://blog.csdn.net/u010771890/article/details/103224004

6. tcp wait_time: 确保来自连接先前化身的重复分组已经在网络中消失 时间为2MSL时间

7. http报文结构：请求行、header、 message-body

8. tcp报文结构 ：

   ![](image\interview\tcpbaowenjiegou.webp)

9. api-server并发问题（etcd需要锁）：通过更改modifiedindex（ResourceVersion）字段进行乐观控制

10. 服务治理等级（qos）：不设置limit和request级别为besteffort、limit和request不等Burstable limit等于request guaranteed  只设置一个值默认request和limit相等

11. 独占cpu设置： 只有 `Guaranteed` pod 中，指定了整数型 CPU `requests` 的容器，才会被分配独占 CPU 资源。 

12. pod几种状态（生命周期）：pending、running、succeeded、Failed、Unknown。

13. 容器状态： waiting、 Running、 Terminated 容器重启策略：Always、Onfailure、Never

14. pause容器作用：业务容器共享pause容器的网络栈和挂载卷

15. namespace： UTS（主机名域名）、IPC（信号量）、PID（进程编号）、NETWORK、MOUNT（挂载点文件系统）、USER（用户和用户组）
...
17. go实现集中限流方法
    ...
    参考文章：https://studygolang.com/articles/23725

18. cpu limit 和 request 设置的区别

    参考文章：https://fuckcloudnative.io/posts/understanding-resource-limits-in-kubernetes-cpu-time/

19. 内存寻址

    https://liam.page/2016/05/01/Introduction-to-Memory-Addressing/

20. calico复用etcd 原理 写入规则 等

21. node ping  pod

22. tcp 状态码 

23. rename原理 docker -v  使用rename 会发生什么
...
26. cpu 设置limit 2  在pod内部看到会有几个cpu
...
### 2.用 500 行 Golang 代码实现高性能的消息回调中间件

https://ruby-china.org/topics/34240

### 3.golang实现hashmap，自动伸缩

https://blog.csdn.net/NewGirlFriend/article/details/79542185

https://leetcode-cn.com/problems/design-hashmap/solution/go-shi-xian-hashmap-by-natty/

### 4.Go语言实现RPC

https://juejin.cn/post/6844903661127090190
...
- 理解go协程、channel、内存分配、GC等基本原理及部分源码
- go GMP（源码级分析）
...
源码位置：$GOROOT/src/cmd/compile/internal/gc/escape.go
...
https://juejin.cn/post/6844903972768055304#heading-119
